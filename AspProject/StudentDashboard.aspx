...
<body>
    ...
    <div class="container">
       ...
       <div class="center">
        <div><p class="text" style="text-align: center">Available Courses</p></div>
        <asp:GridView ID="grdCourse" runat="server" DataSourceID="CourseSqlDataSource" AutoGenerateColumns="False" DataKeyNames="id,teacher_id" OnRowCommand="grdCourse_RowCommand"  CssClass="table table-hover table-striped" GridLines="None" >
            <Columns>
...
WHERE Course.id NOT IN (Select course_id FROM Enroll WHERE student_id = @studentId)" ProviderName="System.Data.SqlClient">
    ...
    <asp:Parameter Name="studentId"  />
       ...
       <div><p class="text" style="text-align:center;">Enrolled Courses</p></div>
        <asp:GridView ID="grdEnrolled" runat="server" AutoGenerateColumns="False" DataKeyNames="id" DataSourceID="EnrolledDataSource" OnRowCommand="grdEnrolled_RowCommand"  CssClass="table table-hover table-striped" GridLines="None" >
            <Columns>
                                <asp:BoundField DataField="name" HeaderText="Name" SortExpression="name" />
...
WHERE Course.id IN (Select course_id FROM Enroll WHERE student_id = @studentId)">
        ...
        <br />
     <asp:Button ID="btnLogout" runat="server" Text="Logout" OnClick="btnLogout_Click" class="button" style="width:100%; padding: 20px;"/>
        </div>
    </div>
